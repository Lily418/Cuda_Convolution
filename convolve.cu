...
__global__ void convolve_optimised(float* data_in, float* data_out, float* kernel, int kernelSize, int BLOCK_SIZE)
{
    ...
    data_in_shared[pos] = data_in[pos];
    if(tx == 0){
        for(int i = pos + BLOCK_SIZE; i < pos + BLOCK_SIZE + kernelSize - 1; i++){
            data_in_shared[i] = data_in[i];
        }
    }
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < ITERS; i++)
    {
        switch(atoi(argv[1])){
            ...
            case 2:
            convolve_optimised<<<GRID_SIZE, BLOCK_SIZE, (BLOCK_SIZE + k.size() - 1) * sizeof(float)>>>(d_data_in, d_data_out, d_kernel, k.size(), BLOCK_SIZE);
            break;
